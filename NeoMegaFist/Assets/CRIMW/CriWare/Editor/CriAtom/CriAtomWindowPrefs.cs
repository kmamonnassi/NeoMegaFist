...
namespace CriWare
{
	...
	public class CriAtomWindowInfo
	{
		...
		public class CueInfo : InfoBase
		{
			...
			public CueInfo(string name, int id, string comment, long length, bool isPublic, short numLimits, List<string> categoryNames)
			{
				this.name = name;
				this.id = id;
				this.comment = comment;
						...
						if (acb != null)
						{
							...
							foreach (CriAtomEx.CueInfo cueInfo in cueInfoList)
							{
								...
								if (found == false)
								{
									...
									var newCueInfo = new CueInfo(cueInfo.name, cueInfo.id, cueInfo.userData, cueInfo.length, Convert.ToBoolean(cueInfo.headerVisibility), cueInfo.numLimits, categoryNames);
